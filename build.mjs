...
const gaMeasurementId = "G-PBK6CCCD7B";
...
for (const name of htmlFiles) {
  ...
  let html = fs
    ...
    .replaceAll(token, effectiveOrigin);

  if (
    indexable &&
    name !== "404.html" &&
    !html.includes(gaMeasurementId)
  ) {
    html = html.replace("</head>", `${gaTag}</head>`);
  }
...
fs.writeFileSync(
  ...
  [
    ...
    ...publicPaths.map(
      p => `  <url><loc>${effectiveOrigin}${p}</loc></url>`
    ),
    "</urlset>",
    ""
  ].join("\n")
...
for (const name of fs.readdirSync(dist)) {
  ...
  if (
    ![".html", ".xml", ".txt", ".json", ".svg"].includes(ext)
  ) {
    continue;
  }
  ...
  if (str.includes(token)) {
    errors.push(`${name}: unresolved site-origin token`);
  }

  if (str.includes("CONTACT_EMAIL_PLACEHOLDER")) {
    errors.push(`${name}: unresolved email placeholder`);
  }
